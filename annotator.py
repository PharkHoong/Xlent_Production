...
"""
    ...
    def generate_data_yaml_after_split(self, folder_path):
        """Generate data.yaml using class names from image filenames"""

        class_map = self.get_class_map_from_filenames(folder_path)
    ...
    """
        ...
        # Keep classes.txt in sync
        self.save_class_names_to_file(folder_path, class_map)
    ...
    def save_class_names_to_file(self, folder_path, class_map):
        ...
        with open(classes_txt_path, "w", encoding="utf-8") as f:
            for _, class_name in sorted(class_map.items()):
    ...
    def get_class_map_from_filenames(self, folder_path):
        """
        Extract class_id → class_name from filenames like:
        0_partid.bmp

        Works BOTH before and after dataset split
        """
        class_map = {}

        search_dirs = [
            folder_path,  # before split
        ...
        for img_dir in search_dirs:
            ...
            for file in os.listdir(img_dir):
                ...
                name = os.path.splitext(file)[0]  # 0_partid
                if "_" not in name:
                    continue

                cls_id_str, cls_name = name.split("_", 1)

                if not cls_id_str.isdigit():
                    continue
        ...
        if not class_map:
            raise ValueError("No valid <id>_<name> filenames found")
